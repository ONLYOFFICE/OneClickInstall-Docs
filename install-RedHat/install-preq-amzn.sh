...
dnf clean all
dnf -y install nano policycoreutils-python*
...
if [ "$INSTALLATION_TYPE" != "COMMUNITY" ]; then
    ...
    PSQL_AVAILABLE_VERSION=$(dnf list postgresql\*-server --available 2>/dev/null | awk '/^postgresql[0-9]+-server/ {gsub("postgresql|-server.*","",$1); print $1}' | sort -nr | head -1)
    ...
    # configure Valkey
    [ -e "/etc/valkey.conf" ] && sed -E -i "s_^bind.*_bind 127.0.0.1_; /^save\s[0-9]+/d" "/etc/valkey.conf"
    ...
    for SVC in valkey rabbitmq-server postgresql; do
