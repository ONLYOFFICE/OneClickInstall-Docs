...
| 🚀 [Start](#-quick-start) | 🛠️ [Editions](#-install-a-specific-edition) | 💡 [Examples](#-examples) | 🖥️ [Reqs](#-system-requirements) | ✅ [OS](#-supported-operating-systems) | 📚 [Resources](#-additional-resources) | 📝 [License](#-license) |
|--------------------------|--------------------------------------------|----------------------------|----------------------------------------|----------------------------------------|----------------------------------------|----------------------|
...
| Flag | Value&nbsp;placeholder | Description | Applies to |
|------|------------------------|-------------|------------|
| `--installation_type` | `COMMUNITY \| ENTERPRISE \| DEVELOPER` | Choose edition | All |
| `--update` | `true \| false` | Update existing components | All |
| `--skiphardwarecheck` | `true \| false` | Skip hardware check | All |
| `--jwtenabled` | `true \| false` | Enable JWT validation | All |
| `--jwtheader` | `<HEADER_NAME>` | JWT HTTP header (**Authorization**) | All |
| `--jwtsecret` | `<JWT_SECRET>` | JWT secret key | All |
| `--localscripts` | `true \| false` | Run local scripts / offline mode | All |
| `--docsport` | `<PORT>` | Docs port (**80**) | All |
| `--documentimage` | `<name> \| <path>` | Image name **or** `.tar.gz` path | Docker |
| `--documentversion` | `<VERSION_TAG>` | Image tag / version | Docker |
| `--installdocumentserver` | `true \| false \| pull` | Install/update Docs or just pull images | Docker |
| `--registry` | `<URL>` | Docker registry URL (e.g. `https://reg.example.com:5000`) | Docker |
| `--username` | `<USERNAME>` | Docker registry username | Docker |
| `--password` | `<PASSWORD>` | Docker registry password | Docker |
| `--useasexternalserver` | `true \| false` | Expose Docs externally (**true**) | Docker |
| `--skipversioncheck` | `true \| false` | Skip version check during update | Docker |
| `--letsencryptdomain` | `<DOMAIN>` | Domain for Let's Encrypt cert | Docker |
| `--letsencryptmail` | `<EMAIL>` | Admin email for Let's Encrypt | Docker |

> Applies to:  
> - `All` - works for both Docker and package installs  
> - `Docker` - applies only to Docker installation  
> - `Package` - applies only to .deb / .rpm installation
...
**Enterprise** and **Developer** editions require a valid commercial license.
